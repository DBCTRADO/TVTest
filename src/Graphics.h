...
namespace TVTest
{
	...
	namespace Graphics
	{
		...
		enum {
			...
			TEXT_FORMAT_HORZ_CENTER		= 0x0002U,
			TEXT_FORMAT_HORZ_ALIGN_MASK	= 0x0003U,
			TEXT_FORMAT_TOP				= 0x0000U,
			TEXT_FORMAT_BOTTOM			= 0x0004U,
			TEXT_FORMAT_VERT_CENTER		= 0x0008U,
			TEXT_FORMAT_VERT_ALIGN_MASK	= 0x000CU,
			...
			TEXT_DRAW_CLEARTYPE			= 0x0400U,
			TEXT_DRAW_HINTING			= 0x0800U,
			TEXT_DRAW_PATH				= 0x1000U
		};
		...
		public:
			...
			bool FillGradient(const CColor &Color1,const CColor &Color2,
							  const RECT &Rect,GradientDirection Direction);
			bool DrawText(LPCTSTR pszText,const LOGFONT &lf,
						  const RECT &Rect,CBrush *pBrush,UINT Flags);
			bool GetTextSize(LPCTSTR pszText,const LOGFONT &lf,
							 UINT Flags,SIZE *pSize);
			bool DrawOutlineText(LPCTSTR pszText,const LOGFONT &lf,
								 const RECT &Rect,CBrush *pBrush,
								 const CColor &OutlineColor,float OutlineWidth,
								 UINT Flags);
			bool GetOutlineTextSize(LPCTSTR pszText,const LOGFONT &lf,
